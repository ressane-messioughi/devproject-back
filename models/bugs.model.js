...
const findAllByProject = async (project_id) => {
  const sql = `SELECT b.id_bug, b.title, b.description, b.status, b.file_url, b.created_at, b.project_id, b.created_by,
...
const create = async (title, description, status, file_url, project_id, created_by) => {
  ...
  const [result] = await db.execute(sql, [title, description, status, file_url, project_id, created_by]);
  return result;
};

// Fonction pour modifier le titre et la description d'un bug (uniquement l'auteur, voir bugs.service.js)
const updateDetails = async (id_bug, title, description) => {
  const sql = 'UPDATE bug SET title = ?, description = ? WHERE id_bug = ?';
  const [result] = await db.execute(sql, [title, description, id_bug]);
  return result;
};
